<template>
  ...
  <div class="one">
    <!-- 顶部编辑按钮及其弹出框 -->
    <div class="p" >
      <el-button type="text" class="p2" @click="edit"><i class="el-icon-edit-outline"></i> 编辑页面</el-button>
      <!-- 点击编辑按钮弹出的页面 -->
      <el-dialog title="" :visible.sync="dialogFormVisible" width="1080px" top="50px">
        <div class="right1">
          ...
          <div class="el-dialog-div">
              ...
              <p class="tit1">面试问题设置</p>
                ...
                <el-table :data="form.dynamicItem"  stripe border style="width: 85%">
                  ...
                  <el-table-column label="问题" width="200px" align="center">
                    <template slot-scope="scope">
                      <el-input type="textarea" :rows="2" resize="none"  maxlength="50" show-word-limit placeholder="请输入问题" v-model="scope.row.question" required="required"></el-input>
                    </template>
                  </el-table-column>
            ...
            <div class="inform1">
              ...
              <div class="details">
                <p class="top">面试通知</p>
                <p class="concrete"><el-input style="width:123px" placeholder="请输入内容"  v-model="informEdit.a"></el-input>{name},{department}邀请你进行{number}面面试。面试时间:{time};面试场地:{location}<el-input  style="width:450px" placeholder="请输入内容"  v-model="informEdit.b" ></el-input></p>
              </div>
            </div>
            <div class="inform2">
              ...
              <el-tabs type="border-card" class="result">
                <el-tab-pane label="成功">
                  <p class="success"><el-input style="width:123px" placeholder="请输入内容(必填)"  v-model="successEdit.a"></el-input>{name}<el-input style="width:250px" placeholder="请输入内容(必填)"  v-model="successEdit.b"></el-input>{department}<el-input style="width:315px" placeholder="请输入内容(必填)"  v-model="successEdit.c"></el-input>{round}面试<el-input style="width:325px" placeholder="请输入内容(选填)"  v-model="successEdit.d"></el-input></p>
                </el-tab-pane>
                <el-tab-pane label="失败">
                  <p class="success"><el-input style="width:123px" placeholder="请输入内容(必填)"  v-model="failEdit.a"></el-input>{name}<el-input style="width:250px" placeholder="请输入内容(必填)"  v-model="failEdit.b"></el-input>{department}<el-input style="width:315px" placeholder="请输入内容(必填)"  v-model="failEdit.c"></el-input>{round}面试<el-input style="width:325px" placeholder="请输入内容(选填)"  v-model="failEdit.d"></el-input></p>
                </el-tab-pane>
              </el-tabs>
            </div>
          </div>
    ...
    <!-- 左边导航条 -->
    <div class="left1">
      <div class="nav1"><a href="#one" :class="['text',isSelect1()?'blue':'']">面试问题设置</a></div>
      <div class="nav1"><a href="#two" :class="['text',isSelect2()?'blue':'']">面试文案编辑</a></div>
      <div class="nav1"><a href="#three" :class="['text',isSelect3()?'blue':'']">结果通知编辑</a></div>
    </div>
  ...
  methods: {
      ...
      get1
        .then((res)=>{
          if(res.data.data.interviewList.length==0){
            this.form.dynamicItem=[{orderNumber:1,question:'请编辑',score:'0',standard:'请编辑'}]
          }else{
            this.form.dynamicItem=res.data.data.interviewList;
          }
        })
    ...
    resetInterviewInform(){
      let sendData2={
        ...
        round:this.round,
        type:1,
        messageTemplate: this.informEdit.a+'{name},{department}邀请你进行{number}面面试。面试时间:{time};面试场地:{location}'+this.informEdit.b,
      }
    ...
    resetSuccessResult(){
      let sendData3={
        ...
        round:this.round,
        type:2,
        messageTemplate:this.successEdit.a+'{name}'+this.successEdit.b+'{department}'+this.successEdit.c+'{round}面试'+this.successEdit.d
      }
    ...
    resetFaultResult(){
      let sendData4={
        ...
        round:this.round,
        type:3,
        messageTemplate: this.failEdit.a+'{name}'+this.failEdit.b+'{department}'+this.failEdit.c+'{round}面试'+this.failEdit.d
      }
    ...
    resetData(){
      ...
      if(this.successEdit.a==''||this.successEdit.b==''||this.successEdit.c==''){
        this.successEmpty=true
      }
      if(this.failEdit.a==''||this.failEdit.b==''||this.failEdit.c==''){
        this.failEmpty=true
      }
      if(this.questionEmpty==true||this.scoreEmpty==true||this.standardEmpty==true){
