  ...
  <div>
    ...
    <!-- 弹窗主体内容 -->
      <div class="all">
        ...
        <div class="registerTable">
          ...
          <div class="one">
            ...
            <el-form :model="ruleForm" ref="ruleForm" label-width="55px" class="demo-ruleForm form" >
              ...
              <el-form-item label="学院" prop="academy" v-if="ruleForm.academy!=null">
                <el-input v-model="ruleForm.academy" disabled></el-input>
              </el-form-item>
              <el-form-item label="专业" prop="major" v-if="ruleForm.major!=null">
                <el-input v-model="ruleForm.major" disabled></el-input>
              </el-form-item>
              <el-form-item label="班级" prop="classNum" v-if="ruleForm.classNum!=null">
        ...
        <div class="reply">
          <div class="replyItem" v-for="(item,index) in reply" :key="index">{{item.time}}&nbsp;&nbsp;&nbsp;&nbsp;{{item.answer}}</div>
        </div>
        <!-- 签到 -->
        <div class="bigTitle">签到</div>
        <div class="reply">
          <div class="replyItem" v-for="(item,index) in signData" :key="index">{{item.time}}&nbsp;&nbsp;&nbsp;&nbsp;{{item.status}}</div>
        </div>
        <!-- 面试安排 -->
        ...
        <div class="evaluate">
          ...
          <div v-for="(item,index) in roundnum" :key="index">
            <!-- 一面二面 -->
            <div class="interview" @click="getDetail(item)"> {{item}}面 <i class="el-icon-thumb"></i> </div>
            <!-- 内部细节 -->
            <div class="detail" v-if="isShowdetail==item">
              <div class="score">
                <table  border width="82%" cellspacing="0">
                  ...
                  <tr>
                    <th>面试问题/面试官</th>
                    <!-- 渲染名字 -->
                    <th v-for="(item,index) in evaluation" :key="index">{{item.name}}</th>
                    <th>排名</th>
                  </tr>
                  <tr v-for="(item,index) in question" :key="index">
                    <td>{{item.questionName}}</td>
                    <td v-for="(item,index) in item.evaluation" :key="index">{{item.score}}</td>
                    <td>{{item.rank}}</td>
                  </tr>
                </table>
              </div>
              ...
              <div class="comment">
                <table  border width="82%" cellspacing="0">
                  ...
                  <tr v-for="(item,index) in question" :key="index">
                    <td>{{item.questionName}}</td>
                    <td><span v-for="(item,index) in item.evaluation" :key="index">{{item.comment}}<br/></span></td>
                  </tr>
                </table>
              </div>
            </div>
      ...
      <span slot="footer" class="dialog-footer">
        <el-button type="primary" @click="resumeDialogVisible = false">关 闭</el-button>
      </span>
    </el-dialog>
  </div>
</template>
      ...
      get
        .then((res) => {
          ...
          let data =[
               {
                  "time": 1254449500,
                  "message": "欢迎选择并愿意加入协会，请于2022年8月31号 19:30在7-115参加第一轮面试。"
              },
              {
                  "time": 1625949500,
                  "message": "(≧^.^≦)欢迎选择并愿意加入协会，请于2022年8月31号 19:30在7-115参加第一轮面试。"
...
.all{
  ...
  // 面试安排
  .arrange,.inform,.evaluate,.result{
    margin-top: 10px;
    border-left: 2px solid gray;
    .table{
      margin-left: 20px;
